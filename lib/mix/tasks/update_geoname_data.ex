defmodule Mix.Tasks.UpdateGeonameData do
  ...
  defp process_geonames_file(filename) do
    tab = :binary.compile_pattern("\t") # BINARY
  ...
  defp reduce_chunk(row, result) do
    case row do
      [geoname_id, name, _, _, _, _, feature_class, _, country_code | _rest] when feature_class in ["P", "A"] -> # feature classes defined here: http://download.geonames.org/export/dump/
